...
$stmt = $pdo->prepare("SELECT * FROM listing WHERE listing_id = ?");
$stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE listing SET title = :title, created_by = :created_by, description = :description, price = :price, image = :image WHERE listing_id = :id");
        $stmt->execute([
        ':title' => $title,
