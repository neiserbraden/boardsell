...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM listing WHERE listing_id = ?");
    $stmt->execute([$id]);
      ...
      <?php foreach ($rows as $row): ?>
        <tr>
          ...
          <td style="white-space: nowrap;">
            <a href="edit.php?id=<?= (int)$row['listing_id'] ?>" class="btn btn-sm btn-secondary">Edit</a>
            <form method="post" class="d-inline">
              <input type="hidden" name="delete_id" value="<?= (int)$row['listing_id'] ?>">
